...
def get_posts(db: Session = Depends(get_db), limit: int=10, skip: int=0, search: Optional[str]=""):
    posts = db.query(models.Post, func.sum(models.Vote.direction).label("Vote")
    ).join(models.Vote, models.Post.id==models.Vote.post_id, isouter=True).group_by(
    models.Post.id, models.Vote.post_id).order_by(models.Post.id).all()
    return posts
...
def delete_post(id:int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail=f"can not alter other user posts")
